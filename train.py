...
def train_model(cfg: DictConfig, device:torch.device):
    ...
    for epoch in range(1,cfg.train.epochs+1):
        ...
        f1_metric.reset()
        if dist.get_rank()==0:
            pbar = tqdm(total = len(train_loader)*torch.cuda.device_count(),unit="batch",dynamic_ncols=True)
    ...
    if dist.get_rank()==0:
        ...
        torch.save(ddp_model.state_dict(),cfg.train.model_path+f"model-epoch:{cfg.train.epochs}-{dt}.pth")
